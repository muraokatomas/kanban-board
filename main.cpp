...
Card* CrearTareaConfiguracionAmbienteDesarrollo(string id)
{
	Card* card = KanbanBoardCard::CreateCard(id, "Configurar el Ambiente de desarrollo.", KanbanBoardCard::Story);
...
Card* CrearTareaImplementacionCard(string id)
{
	...
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Estudiante", "¿Qué?"));
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Profe", "La descripción de esta card esta en un formato particular,  conocido como Story Card. Las stories card sirven en principio para iniciar una conversación, y poder definir los requerimientos de una funcionalidad. Si prestan atención en formato es el siguiente: Como X quiero poder hacer Y para Z. X representa un interesado en una funcioanalidad (rol de un usuario del sistema). Y representa una funcionalidad. Z representa el valor que agrega al negocio esa funcionalidad, es decir el porque es importante esa funcionalidad"));
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Estudiante", "¿A qué se refiere con \"iniciar una conversación\"?"));
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Profe", "Por ejemplo, luego de que esta tarjeta fue escrita, el equipo de desarrollo podría comenzar una conversación sobre el alcance de esta Story Card y decidir: Creemos un TDA Card, declaremos su interfaz y validemosla con el product owner, luego la implementamos."));
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Estudiante", "¿Qué es un product owner?"));
	KanbanBoardCard::AddComment(card, KanbandBoardCardComment::CreateCardComment("Profe", "Product owner es un rol que define a una persona muy importante en un proyecto: La persona que más sabe del producto que se desarrolla desde la perspectiva del usuario, es el representante del usuario dentro del equipo y es el que puede definir el alcance y las prioridades de las Story Card"));
	return card;
}

Card* CrearTareaAgregarComentariosACard(string id)
{
	Card* card = KanbanBoardCard::CreateCard(id, "Implementar el TDA Card Comment.", KanbanBoardCard::Story);
	KanbanBoardCard::SetDescription(card, "Como un miembro del equipo de trabajo de un proyecto quiero poder agregar un comentario a una Card para agregar inforamcion o realizar preguntas a algún miembro del equipo y que esto quede como parte de la historia de la Card");
	return card;
}

Card* CrearTareaAgregarEstadosCard(string id)
{
	Card* card = KanbanBoardCard::CreateCard(id, "Implementar el TDA State.", KanbanBoardCard::Story);
	return card;
}

Card* CrearTareaAgregarColumna(string id)
{
	Card* card = KanbanBoardCard::CreateCard(id, "Implementar el TDA Column.", KanbanBoardCard::Story);
	return card;
}
...
void MoverTareasSegundaEntregaAColumnaEnProgreso(Column* backlog, Column* enProgreso)
{
	KanbanBoardCard::Card* tareasPrimerEntrega[2];
...
void MoverTareasPrimerEntregaAColumnaTerminado(Column* backlog, Column* terminado)
{
	KanbanBoardCard::Card* tareasPrimerEntrega[3];
...
void MostrarColumna(Column* columna)
{
	...
	KanbanBoardCard::Card* proximaTareaPrioritaria = KanbanBoardColumn::GetTopPriorityCard(columna);
	
	while (proximaTareaPrioritaria != nullptr)
	{
